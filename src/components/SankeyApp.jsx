...
// --- Components ---
const CalculatedField = ({ label, value, icon: Icon, colorVar, isMonthly }) => {
...
const ToggleField = ({ label, value, isActive, onToggle, icon: Icon, colorVar, isMonthly, hint }) => {
    const displayValue = isMonthly ? Math.round(value / 12) : value;
    return (
        <div className="sankey-field">
            <div className="sankey-field-header">
...
const InputField = ({ label, field, icon: Icon, colorVar, data, isMonthly, updateField, hint }) => {
...
const CategoryField = ({ label, value, icon: Icon, colorHex, isMonthly, onChange, onRemove, onGripEnter, onGripLeave }) => {
    const displayValue = isMonthly ? Math.round(value / 12) : value;
    const smallStep = isMonthly ? 10 : 100;
    const largeStep = isMonthly ? 50 : 500;
...
export default function SankeyApp() {
    ...
    const handleAIAnalysis = async () => {
        ...
        const rootCats = data.lifestyleCategories.filter(c => !c.parentId);
        const totalLifestyle = rootCats.reduce((sum, cat) => sum + cat.value, 0);
